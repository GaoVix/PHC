...
class MotionLib():
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        extend_body_parent_ids = torch.tensor(0, device=self._device)
        extend_body_pos_in_parent = torch.tensor([[0.0, 0.0, 0.4]]).repeat(body_rot.shape[0], 1, 1).to(self._device)
        print(body_rot[:, extend_body_parent_ids].reshape(-1, 4).shape)
        print(extend_body_pos_in_parent.reshape(-1, 3).shape)
